...
- 消息历史管理
  - 目前所有消息都存放在内存中的 `std::vector<ChatMessage> messages_` 中（见 server/include/chatroom_server.h），没有淘汰策略也没有持久化。
  - 可改进方向：
    - 为消息历史增加条数上限，只保留最近 N 条消息；
    - 或引入简单的持久化（日志文件/SQLite），支持服务重启后恢复历史记录。
- 获取消息接口设计
  - `/messages` 当前总是返回所有消息，客户端通过本地计数 `last_message_count_` 过滤“新消息”（见 client/src/chatroom_client.cpp）。
  - 可改进方向：
    - 在服务端支持 `?since=` 等查询参数，只返回增量消息；
    - 消息索引或时间戳由服务器管理，客户端只需传增量条件。
- 登录和用户会话
  - 登录仅依赖用户名，没有任何认证和重复用户名策略，`handleLogin` 只返回一个 `connection_id`。
  - 可改进方向：
    - 增加用户名合法性校验（非空、最大长度、字符集限制），处理重名用户（如自动加序号）；
    - 引入简单 token 或会话标识，用于后续发送消息、心跳时做身份校验。
- 在线用户展示
  - 服务端已提供 `/users` 和心跳机制，可计算 online/idle_seconds，但当前 CLI 客户端未暴露对应命令。
  - 可改进方向：
    - 在客户端增加 `/users` 等命令，调用 `/users` 接口展示在线用户和心跳时间。
