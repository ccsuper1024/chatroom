...
bool ServerConfig::load(const std::string &config_file) {
  ...
  while (std::getline(in, line)) {
    ...
    try {
      if (key == "port") {
        ...
        db.db_name = value;
      } else if (key == "db_pool_initial") {
        db.initial_size = std::stoi(value);
      } else if (key == "db_pool_max") {
        db.max_size = std::stoi(value);
      }
    } catch (...) {
      std::cerr << "Error parsing config key: " << key << ", value: " << value
                << std::endl;
    }
  }
  ...
  if (thread_pool.max_threads == 0) {
    ...
    thread_pool.max_threads = std::max<std::size_t>(
        thread_pool.core_threads,
